...
class Game {
  ...
  populatePreraceCards() {
    ...
    container.innerHTML = this.race.horses
      .map((horse, i) => {
  ...
  smoothScrollTo(
    ...
    const ease = linear
      ? easings.linear
      : easings[easingName] || easings.easeOutCubic;
