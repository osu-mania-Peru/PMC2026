...
export default function PreviewPanel({ isOpen, onClose, map, apiBaseUrl }) {
  ...
  useEffect(() => {
    ...
    const fetchPreviewData = async () => {
      ...
      try {
        const response = await fetch(
          `${apiBaseUrl}/mappools/preview/${map.beatmapset_id}`
        );

        if (!response.ok) {
          if (response.status === 404) {
            throw new Error('Beatmapset not downloaded on server');
          }
    ...
    fetchPreviewData();
  }, [map?.beatmapset_id, isOpen, apiBaseUrl]);
